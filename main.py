...
def pick_emoticons(emotion, emots):
    """Pick a random emoticon of the specified emotion"""
...
def process_image(url = None, urldata = None, key = '6e683b8651bf495292f2eda0e69ac4b2', json = True, params = urllib.urlencode({}), render = True, show = False, **kwargs):
    """Takes a local image or URL, submit to Microsoft Emotion API"""
...
def render_image(img = None, url = None, urldata = None, annotation = None, json = True, show = True):
    """Overlay emoticons specified by the annotation generated by process_image"""
    if img is None:
        if url is not None and json:
    ...
    for currFace in annotation:
        ...
        if y < -w:
            ...
            y += w
        _emot = pick_emoticons(currEmotion, emots).resize((w, w))
...
def render_video(url, outfile = "tmp.mp4", fps = 15, prevlayer = None, passes = 1, fadeout = None, fadein = None, step = None):
    """Adds emoticons at various time intervals and add as overlays"""
    ...
    for p in range(passes):
        if step is None:
            tslices = np.linspace(np.random.uniform(0, 5), vid.duration - np.random.uniform(0, 0.5), num = np.random.randint(4, 20))
        else:
            tslices = np.linspace(np.random.uniform(0, 0.5), vid.duration - np.random.uniform(0, 0.5), num = int(vid.duration / step))
...
def webcam_get(frame):
    """Worker for calling API"""
    urldata = cv2.imencode(".png", frame)[1].tobytes()
    return process_image(urldata = urldata, render = False, json = False)

def webcam_video():
    """Starts a webcam session with realtime overlay of emoticons"""
    ...
    while rval:
        if ctr == 0:
            f = executor.submit(webcam_get, frame = frame)
        if not f.running():
            ann = f.result()
            f = executor.submit(webcam_get, frame = frame)
        frame = Image.fromarray(cv2.cvtColor(frame, cv2.COLOR_BGR2RGB))
        if ann is not None:
            frame = render_image(img = frame, annotation = ann, show = False)
...
if __name__ == "__main__":
    ...
    a = process_image("targets/testtarget.jpg", json = False, show = False)
    a
    a = process_image("targets/target2.png", json = False, show = False)
    a
    a = process_image("targets/target3.jpg", json = False, show = False)
    a
    
    #vidname = ["anger_hitler_1", "sadness_rocky", "sadness_tobey", "sadness_tomcruise", "surprise_happiness_kid"]
    vidname = ["sadness_tobey"]
    for vidname in vidname:
         vv, ll = render_video("targets/" + vidname + ".mp4", outfile = "targets/example.mp4", passes = 1, step = 3, fadeout = 2, fps = 20)
    
    pygame.display.set_caption('Hello World!')
    clip = VideoFileClip("targets/anger_hitler_1tagged.mp4")    
    clip.preview()
    pygame.quit()
